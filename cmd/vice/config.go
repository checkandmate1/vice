...
type ConfigNoSim struct {
	...
	// DisplayRoot is rebuilt dynamically from the pane instances above.
	// We still serialize it for now but don't rely on it for state.
...
func LoadOrMakeDefaultConfig(lg *log.Logger) (config *Config, configErr error) {
	...
	if contents, err := os.ReadFile(fn); err == nil {
		...
		// Ensure all panes are initialized
		config.ensurePanesInitialized()
...
// ensurePanesInitialized ensures that all pane instances are initialized,
// even if they were not present in the old DisplayRoot or if they were nil.
func (c *Config) ensurePanesInitialized() {
	if c.STARSPane == nil {
	...
	if c.SplitLinePositions[0] == 0 {
		c.SplitLinePositions[0] = 0.8
	}
	if c.SplitLinePositions[1] == 0 {
		c.SplitLinePositions[1] = 0.075
	}
}
...
func (c *ConfigNoSim) buildDisplayRoot(radarPane panes.Pane) *panes.DisplayNode {
	// Create the messages and radar panes split
	messagesNode := &panes.DisplayNode{Pane: c.MessagesPane}
	radarNode := &panes.DisplayNode{Pane: radarPane}
	
	leftSplit := &panes.DisplayNode{
		SplitLine: panes.SplitLine{
			Pos:  c.SplitLinePositions[1], // Y split
			Axis: panes.SplitAxisY,
		},
		Children: [2]*panes.DisplayNode{messagesNode, radarNode},
	}
	
	// Create the flight strip pane
	flightStripNode := &panes.DisplayNode{Pane: c.FlightStripPane}
	
	// Create the root split
	return &panes.DisplayNode{
		SplitLine: panes.SplitLine{
			Pos:  c.SplitLinePositions[0], // X split
			Axis: panes.SplitAxisX,
		},
		Children: [2]*panes.DisplayNode{leftSplit, flightStripNode},
	}
}

func (c *Config) Activate(r renderer.Renderer, p platform.Platform, eventStream *sim.EventStream, lg *log.Logger) {
	// Determine which radar pane to use based on sim type
	isSTARSSim := c.Sim != nil && c.Sim.State.TRACON != ""
	c.RebuildDisplayRootForSim(isSTARSSim)
