...
type ManeuverComplete struct {
	...
	Heading float32       // target heading (UntilHeading)
...
type LateralManeuver struct {
	Heading        float32       // magnetic heading to fly
...
func (nav *Nav) maneuverGetHeading(wxs wx.Sample, simTime time.Time) (float32, av.TurnDirection, float32) {
	m := &nav.Heading.Maneuvers[0]

	// Compute the heading for this maneuver
	heading := m.Heading
	if !m.FlyToward.IsZero() {
		heading = math.Heading2LL(nav.FlightState.Position, m.FlyToward,
...
func makeStandard45Maneuver(nav *Nav, wp []av.Waypoint, exitAlt *float32) []LateralManeuver {
	...
	inboundHdg := math.Heading2LL(wp[0].Location, wp[1].Location, nmPerLong, magVar)
	outboundHdg := math.OppositeHeading(inboundHdg)
	awayHdg := math.NormalizeHeading(outboundHdg + float32(util.Select(pt.RightTurns, -45, 45)))
...
func makeRacetrackManeuver(nav *Nav, wp []av.Waypoint, exitAlt *float32) []LateralManeuver {
	...
	acFixHdg := math.Heading2LL(nav.FlightState.Position, fixLoc, nmPerLong, magVar)
	...
	case av.TeardropEntry:
		tearHdg := math.NormalizeHeading(outboundHdg + float32(util.Select(pt.RightTurns, -30, 30)))
		baseHdg := math.NormalizeHeading(inboundHdg + float32(util.Select(pt.RightTurns, -90, 90)))
	...
	case av.ParallelEntry:
		...
		interceptHdg := math.NormalizeHeading(inboundHdg + float32(util.Select(pt.RightTurns, -30, 30)))
		maneuvers = append(maneuvers,
			// turn outbound
...
func outboundLeg(nav *Nav, pt *av.ProcedureTurn, heading float32, scale float32) LateralManeuver {
